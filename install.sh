...
if [[ -v INSTALLER_LANGS[$l] ]]; then
    ...
    echo 'No such language, fallback to English.'
    INSTALLER_LANG=English
fi
#  break
#done < /dev/tty
...
cp -r $source/{theme.txt,background.png,icons,item*,selected_item*,terminal_box*,*.pf2} .
...
THEME_DIR=/boot/${GRUB_DIR}/themes/${GRUB_THEME}

echo 'Removing GRUB theme directory from previous installation'
sudo rm -rf $THEME_DIR

echo 'Creating GRUB theme directory'
